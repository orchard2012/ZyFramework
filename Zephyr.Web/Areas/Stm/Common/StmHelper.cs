...
namespace Zephyr.Web.Areas.Stm.Common
{
    public class MmsHelper
    {
        ...
        public static dynamic GetEditUrls(string controller,object extend=null)
        {
            ...
            expando["getdetail"] =  string.Format("/api/stm/{0}/getdetail/",controller);
            expando["getmaster"] = string.Format("/api/stm/{0}/geteditmaster/", controller);
            expando["edit"] = string.Format("/api/stm/{0}/edit/", controller);
            expando["audit"] = string.Format("/api/stm/{0}/audit/", controller);
            expando["getrowid"] = string.Format("/api/stm/{0}/getnewrowid/", controller);
        ...
        {
            var expando = (IDictionary<string, object>)new ExpandoObject();
            expando["query"] = string.Format("/api/stm/{0}",controller);
            expando["remove"] = string.Format("/api/stm/{0}/", controller);
            expando["billno"] = string.Format("/api/stm/{0}/getnewbillno", controller);
            expando["audit"] = string.Format("/api/stm/{0}/audit/", controller);
            expando["edit"] = string.Format("/stm/{0}/edit/", controller);
            if (extend!=null)
                EachHelper.EachObjectProperty(extend, (i, name, value) => { expando[name] = value; });
